...
def SharmaNet(input_shape, train_all_baseline=False, classification=True, weight_decay=1e-5, weights='afew',
    ...
    seres50 = SEResNet50(input_shape=(input_shape[2], input_shape[3], input_shape[4]), classes=7)
    # load FER weights
    # pop the last classification layers
    # seres50.summary()
    backbone = Model(seres50.input, seres50.layers[-4].output)
    x = TimeDistributed(backbone, input_shape=input_shape)(input)
    ...
    if "early" in train_mode:
        audio_input = Input(shape=(input_shape[1], audio_dim))
